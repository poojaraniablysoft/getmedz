$(document).ready(function(){
	...
	function initialize(){
		$('.close').click($.systemMessage.close);
	}
	$.extend($.systemMessage, {
		...
		processing: function(){
			$.systemMessage('Processing','alert--process');
		},
		success:function(msg){
			...
			if(append==undefined){
				...
				append='Oops! Internal error. Server says ';
			}
			$.systemMessage(append+ msg,'alert--danger');
		},
		close:function(){
		...
		$(document).on('click', '.linksvertical li a', function(event){		
  ...
  $('.close').click($.systemMessage.close);
    ...
    */
     ...
     /* for fixed/fluid layout */    
		...
		$(".linksvertical li a").click(function(e){
			e.preventDefault();
		});
...
function ask_step(step)
{	
var data = '';
data = $('#frmQuestionForm').serialize();
if(step==3)
{
	callAjax(generateUrl('home', 'step3'),data, function(t){
	if(t)
	{
	 $('#ask-step').html(t);
	}
	else{
		window.location.href = generateUrl('home', 'questionsetup');
	}
	});
}
else if(step==4)
{	
	callAjax(generateUrl('home', 'step4'),data, function(t){
if(t== true)		
{
...
{	
	callAjax(generateUrl('home', 'step5'),data, function(t){	
	 $('#ask-step').html(t);	
	});
}
return false;
...
*/
 function getQuestionForm(step)
{

	 callAjax(generateUrl('home', 'Questionform'),'step=' + step, function(t){
	 if(t == true)
	 {
		 window.location.href = generateUrl('home', '');
	 }
