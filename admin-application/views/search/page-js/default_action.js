...
function loadForm(id){
	if (!id) id=0;	
	showHtmlElementLoading($('#form-div'));
	callAjax(generateUrl('search', 'form'), 'id=' + id + '&outmode=html', function(t){
		$('#form-div').html(t);
                 $('#tab_a_2').click();
	});
...
function submitsearch(frm, v){
  v.validate();
	...
	callAjax(generateUrl('search', 'listing'),data, function(t){
	$('#listing-div').html(t);
...
function list(page){
	if(!page) page=0;
	showHtmlElementLoading($('#listing-div'));
	callAjax(generateUrl('search', 'listing'), 'page='+page, function(t){
	$('#listing-div').html(t);
...
function disable(user_id,mode,ref) {
	...
	callAjax(generateUrl('search', 'change_listing_status'), data, function(t){
