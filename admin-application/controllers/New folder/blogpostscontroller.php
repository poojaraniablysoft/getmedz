...
class BlogpostsController extends BackendController {
    ...
    function listblogposts($page = 1) {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['btn_submit'])) {
			...
			$url = generateUrl('Blogposts', 'listblogposts');			
    ...
    private function getSearchForm() {
        ...
        $fld1 = $frm->addButton('', 'btn_cancel', 'Clear Search', '', ' class="medium" onclick="clearSearch()"');
    ...
    function add() {
        ...
        /* $this->b_crumb->add("Add Post", generateUrl("blogposts", "add"));
    ...
    function edit($blog_post_id) {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (!$frm->validate($post)) {
                ...
                if (!empty($_FILES['post_image_file_name']['name'])) {
                    $image_names = array();
                    if (!$this->saveUploadedPostFiles($_FILES, $image_names)) {
						Message::addErrorMessage('Error: Post Image(s) not uploaded!');
						redirectUser(generateUrl('blogposts', 'edit', array($blog_post_id)));
                    }
					$post['post_image_file_name'] = $image_names;
                }
       ...
       /*  $this->b_crumb->add("Edit Post", generateUrl("blogposts", "edit"));
        $this->set('breadcrumb', $this->b_crumb->output()); */
		...
		if(isset($tempName)){
			...
			if(!is_array($tempName)){
				...
				if ( !( $imgType == "image/gif" || $imgType == "image/jpeg" || $imgType == "image/jpg" || $imgType == "image/png" ) ) {
					Message::addErrorMessage(getLabel('M_ERROR_BLOG_POST_FILE_EXTENSION'));
					return false;
				}
			} else {
		...
		if(isset($imgSize)){
			if(!is_array($imgSize)){
				...
				if ($sizeFile  > 2048 || $sizeFile  < 5) {
					Message::addErrorMessage(getLabel('M_ERROR_BLOG_POST_FILE_SIZE'));
					return false;
				}
			} else {
        ...
        if (is_array($files['post_image_file_name']['name'])) {
            foreach ($files['post_image_file_name']['name'] as $id => $filename) {
                if (is_uploaded_file($files['post_image_file_name']['tmp_name'][$id])) {
                    $saved_image_name = '';
                    if (saveImage($files['post_image_file_name']['tmp_name'][$id], $files['post_image_file_name']['name'][$id], $saved_image_name, 'post-images/')) {
    ...
    protected function getImagesHtml($product_images, $product_id, $image_folder) {
        ...
        if (isset($product_images['imgs']) && is_array($product_images['imgs']) && sizeof($product_images['imgs']) > 0) {
            foreach ($product_images['imgs'] as $id => $img) {
                $photo_html .= '<div class="photosquare"><img alt="" src="' . generateUrl('image', $image_folder, array('thumb', $img),CONF_WEBROOT_URL) . '"> <a class="crossLink" href="javascript:void(0)" onclick="return removeImage(this, ' . intval($id) . ');"></a>';
