...
class BannersController extends BackendController {
    ...
    function __construct($model, $controller, $action) {
        ...
        $this->b_crumb->add("Banners Management", Utilities::generateUrl("banners"));
    }
    ...
    function form($banner_id=0) {
		...
		if($_SERVER['REQUEST_METHOD'] == 'POST' && isset($post['btn_submit'])){
		 ...
		 if ($banner_id > 0) {
            $this->b_crumb->add("Edit Banner", Utilities::generateUrl("banners", "form"));
        } else {
            $this->b_crumb->add("Add Banner", Utilities::generateUrl("banners", "form"));
        }
		$this->set('breadcrumb', $this->b_crumb->output());
        $this->set('frm', $frm);
        $this->_template->render();
    ...
    protected function getForm($banner_id,$data=array()) {
		...
		$fld=$frm->addHtml('<small>*Note:- Banner Dimensions - 1583 X 449.</small>','');
