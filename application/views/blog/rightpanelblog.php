<div class="col_right">
    ...
    <a class="themeBtn" href="<?php echo generateUrl('blog', 'contribution'); ?>" class="grydtn-btn"><?php echo getLabel('L_Contribute')?></a>
    <?php 
     function getList($categories1, $level) {
         $str='';
        $limit = 10; /* level limit */
        $str.='<li><a href="' . generateUrl('blog', 'category', array($categories1['category_seo_name'])) . '">' . ucfirst($categories1['category_title']) . '<span class="countxt">(' . $categories1['count_post'] . ')</span> </a>';
        if (isset($categories1['children'])) {
            $level++;
            if ($level < $limit) {
                $str.='<span class="accordion_icon "></span>';
            }
            $str.='<ul class="sub_cat" style="display:none">';
    ...
    if ($categories || is_array($categories)) { ?>
        <div class="round_sectn">
            <div class="sectnTop">
                <h4><?php echo getLabel('L_Categories');?> </h4>
            </div>
...
//                        echo '<li><a href="' . generateUrl('blog', 'category', array($categories1['category_seo_name'])) . '">' . ucfirst($categories1['category_title']) . '<span class="countxt">' . $categories1['count_post'] . '</span> </a>';
//                        if (isset($categories1['children'])) {
//                            echo '<ul class="sub_cat">';
//                            foreach ($categories1['children'] as $children) {
//                                echo '<li><a href="' . generateUrl('blog', 'category', array($children['category_seo_name'])) . '">' . ucfirst($children['category_title']) . ' <span class="countxt">' . $children['count_post'] . '</span></a> </li>';
//                            }
//                            echo '</ul>';
//                        }
//                        echo '</li>';
    ...
    <?php if ($archives || is_array($archives)) { ?>
        <div class="round_sectn">
            <div class="sectnTop">
                <h4><?php echo getLabel('L_Archives');?></h4></div>
            <div class="sectnMiddle">
                <ul class="blog_lnks">
                    <?php
    ...
    <div class="round_sectn">
        <?php if ($recent_post || is_array($recent_post)) { ?>
            <div class="sectnTop">
                <h4><?php echo getLabel('L_Recent_Posts');?></h4>
            </div>
            <div class="sectnMiddle">
                <ul class="post-links">
                    <?php
                    foreach ($recent_post as $records) {
                      ...
                      <?php if( $records['post_comment_status']!=0 && $records['comment_count'] > 0) {?>
                        <li><a href="<?php echo generateUrl('blog', 'post', array($records['post_seo_name'])); ?>#comment-form"><span><i class="icon ion-chatbox"></i></span><?php echo $records['comment_count']; ?> <?php echo getLabel('L_Comments');?></a></li>
                      <?php } ?>    
                </ul>
                    </li>
                    <?php
