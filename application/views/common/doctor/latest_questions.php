...
if (count($arr_listing) > 0) {
  foreach ($arr_listing as $question) { ?> 
	  ...
	  <div class="dashboard__list-box">
		<div class="question_asked">
		  ...
		  <span class="questio_askedon">
			<span><?php if ($reply['reply_by'] == 0) { ?><?php echo getLabel('L_Asked_On')?> <?php } ?></span>
			<?php  echo date("D, dS M Y h:i T", strtotime($question['order_date'])); ?></span> <a class="button button--fill button--orange" href="<?php echo generateUrl('doctor', 'question', array($question['orquestion_id'])); ?>"><?php echo getLabel('L_Accept_&_Reply');?></a>
		</div>
	  </div>
	  <div class="dashboard__list-box">
		<ul class="question__action">
		  <li><a href="<?php echo generateUrl('doctor','acceptquestion',array($question['orquestion_id']));?>" ><i>
	...
	S9.311,3.077,17,3.077C24.686,3.086,30.913,9.314,30.923,17C30.923,24.689,24.689,30.923,17,30.923z"/>
			...
			</i><span><?php echo getLabel('L_Accept_this_Question');?></span></a></li>
		</ul>
	  </div>
	  <div class="dashboard__list-box">
