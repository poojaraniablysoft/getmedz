 <div class="dashboard__sidebar">
                <div class="dashboard--userinfo">
                  <div class="user_media"> <img src="<?php echo generateUrl('image', 'getCustomerProfilePic', array(Members::getLoggedUserID(),61,61)); ?>" alt=""> </div>
                  <span class="dashboard__username"> <?php echo $customerName;?> </span> </div>
                <div class="dashboard_menu">
                  <h3><?php echo getLabel('L_Menu'); ?> <a title="Menu" class="dashboardToggle js--dashboardToggle" href="javascript:void(0);"><span></span><span></span><span></span></a></h3>
                  <ul>
                    <li class="<?php if(isset($selected_menu) && $selected_menu=='default_action') { ?> active <?php } ?>"><a  href="<?php echo generateUrl('customer'); ?>"><i>
                      <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
	 viewBox="0 0 512.001 512.001" style="enable-background:new 0 0 512.001 512.001;" xml:space="preserve">
                        <g>
                          <g>
                            <path d="M457.637,134.681c-29.976,0-54.363,24.387-54.363,54.363c0,10.933,3.255,21.114,8.831,29.647l-58.733,58.733
			c-8.534-5.576-18.715-8.831-29.648-8.831c-11.329,0-21.858,3.488-30.576,9.441l-59.338-59.347
			c5.575-8.533,8.828-18.713,8.828-29.644c0-29.976-24.387-54.363-54.363-54.363c-29.976,0-54.363,24.387-54.363,54.363
			c0,10.931,3.254,21.108,8.827,29.641L84.004,277.42c-8.532-5.574-18.711-8.827-29.641-8.827C24.387,268.593,0,292.981,0,322.957
			s24.387,54.363,54.363,54.363c29.976,0,54.363-24.387,54.363-54.363c0-10.933-3.255-21.114-8.831-29.648l58.733-58.733
			c8.534,5.576,18.715,8.831,29.648,8.831c10.932,0,21.113-3.255,29.646-8.831l59.666,59.674
			c-5.206,8.338-8.226,18.174-8.226,28.706c0,29.976,24.387,54.363,54.363,54.363s54.363-24.387,54.363-54.363
			c0-10.931-3.254-21.109-8.827-29.641l58.736-58.736c8.533,5.574,18.712,8.827,29.641,8.827c29.976,0,54.363-24.387,54.363-54.363
			C512.001,159.066,487.613,134.681,457.637,134.681z M54.363,354.849c-17.586,0-31.893-14.307-31.893-31.892
			c0-17.586,14.307-31.893,31.893-31.893c17.585,0,31.893,14.307,31.893,31.893C86.256,340.542,71.949,354.849,54.363,354.849z
			 M188.276,220.936c-17.585,0-31.893-14.307-31.893-31.893c0-17.585,14.307-31.893,31.893-31.893
			c17.586,0,31.893,14.307,31.893,31.893C220.169,206.629,205.862,220.936,188.276,220.936z M323.724,354.849
			c-17.585,0-31.893-14.307-31.893-31.893s14.307-31.893,31.893-31.893c17.585,0,31.893,14.307,31.893,31.893
			C355.616,340.542,341.309,354.849,323.724,354.849z M457.637,220.936c-17.585,0-31.893-14.307-31.893-31.893
			c0-17.585,14.307-31.893,31.893-31.893c17.585,0,31.893,14.307,31.893,31.893C489.53,206.629,475.222,220.936,457.637,220.936z"/>
                          </g>
                        </g>
                        <g>
                          <g>
                            <path d="M176.774,272.717c-4.388-4.387-11.501-4.387-15.889,0l-22.854,22.854c-4.387,4.387-4.387,11.501,0.001,15.889
			c2.194,2.194,5.069,3.291,7.944,3.291s5.751-1.098,7.944-3.291l22.854-22.854C181.163,284.219,181.163,277.104,176.774,272.717z"
			/>
                          </g>
                        </g>
                        <g>
                          <g>
                            <path d="M373.962,200.939c-4.388-4.387-11.5-4.387-15.89,0.001l-22.854,22.854c-4.387,4.387-4.387,11.501,0,15.889
			c2.195,2.193,5.07,3.29,7.945,3.29c2.876,0,5.75-1.098,7.944-3.291l22.854-22.854C378.349,212.44,378.349,205.326,373.962,200.939
			z"/>
                          </g>
                        </g>
                      </svg>
                      </i><span class="link"><?php echo getLabel('L_Dashboard');?></span></a></li>
                   
                    <li class="<?php if(!isset($selected_menu) || $selected_menu=='') { ?> active <?php } ?>"><a href="<?php echo generateUrl('customer', 'questions'); ?>"><i>
                      <svg width="64" version="1.1" xmlns="http://www.w3.org/2000/svg" height="64" viewBox="0 0 64 64" xmlns:xlink="http://www.w3.org/1999/xlink" enable-background="new 0 0 64 64">
                        <g>
                          <g fill="#1D1D1B">
                            <path d="M32,0C14.355,0,0,14.355,0,32s14.355,32,32,32s32-14.355,32-32S49.645,0,32,0z M32,60    C16.561,60,4,47.439,4,32S16.561,4,32,4s28,12.561,28,28S47.439,60,32,60z"/>
                            <path d="m32.541,10.434c-7.9,0-14.326,6.426-14.326,14.324 0,1.104 0.895,2 2,2s2-0.896 2-2c0-5.692 4.633-10.324 10.326-10.324 5.691,0 10.322,4.632 10.322,10.324s-4.631,10.324-10.322,10.324c-1.105,0-2,0.896-2,2v7.731c0,1.104 0.895,2 2,2s2-0.896 2-2v-5.871c6.953-0.976 12.322-6.965 12.322-14.185 0-7.898-6.425-14.323-14.322-14.323z"/>
                            <circle cx="32.539" cy="52.404" r="2.189"/>
                          </g>
                        </g>
                      </svg>
                      </i><span class="link"><?php echo getLabel('L_My_Questions');?></span></a></li>
					  <li class="<?php if(isset($selected_menu) && $selected_menu=='ask_question') { ?> active <?php } ?>"><a href="<?php echo generateUrl('home', '').'?ask_question'; ?>"><i>
                      <svg width="64" version="1.1" xmlns="http://www.w3.org/2000/svg" height="64" viewBox="0 0 64 64" xmlns:xlink="http://www.w3.org/1999/xlink" enable-background="new 0 0 64 64">
                        <g>
                          <g fill="#1D1D1B">
                            <path d="M32,0C14.355,0,0,14.355,0,32s14.355,32,32,32s32-14.355,32-32S49.645,0,32,0z M32,60    C16.561,60,4,47.439,4,32S16.561,4,32,4s28,12.561,28,28S47.439,60,32,60z"/>
                            <path d="m32.541,10.434c-7.9,0-14.326,6.426-14.326,14.324 0,1.104 0.895,2 2,2s2-0.896 2-2c0-5.692 4.633-10.324 10.326-10.324 5.691,0 10.322,4.632 10.322,10.324s-4.631,10.324-10.322,10.324c-1.105,0-2,0.896-2,2v7.731c0,1.104 0.895,2 2,2s2-0.896 2-2v-5.871c6.953-0.976 12.322-6.965 12.322-14.185 0-7.898-6.425-14.323-14.322-14.323z"/>
                            <circle cx="32.539" cy="52.404" r="2.189"/>
                          </g>
                        </g>
                      </svg>
                      </i><span class="link"><?php echo getLabel('L_Ask_A_New_Question');?></span></a></li>
                    <li class="<?php if(isset($selected_menu) && $selected_menu=='profile') { ?> active <?php } ?>" ><a href="<?php echo generateUrl('customer', 'profile'); ?>"><i>
                      <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
	 viewBox="0 0 440.248 440.248" style="enable-background:new 0 0 440.248 440.248;" xml:space="preserve">
                        <g>
                          <path d="M126.195,150c41.355,0,75-33.645,75-75s-33.645-75-75-75s-75,33.645-75,75S84.84,150,126.195,150z M126.195,30
		c24.813,0,45,20.187,45,45s-20.187,45-45,45s-45-20.187-45-45S101.382,30,126.195,30z"/>
                          <path d="M356.486,108.82L229.576,235.729C206.944,201.327,168.313,180,126.195,180c-68.236,0-123.75,55.514-123.75,123.75v15
		h144.11l-6.656,6.656l-33.524,114.842l114.842-33.525l216.586-216.585L356.486,108.82z M395.376,190.138l-28.055,28.054
		l-38.891-38.891l28.055-28.054L395.376,190.138z M33.644,288.75C40.845,244.162,79.606,210,126.195,210
		c34.024,0,65.053,18.367,81.578,47.532l-31.218,31.218H33.644z M205.436,380.078l-54.924,16.034l16.033-54.924l140.673-140.673
		l38.891,38.891L205.436,380.078z"/>
                        </g>
                        <g>
                      </svg>
                      </i><span class="link"><?php echo getLabel('L_Edit_Profile');?></span></a></li>
                    <li><a href="<?php echo generateUrl('members', 'logout'); ?>"><i>
                      <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
	 viewBox="0 0 491.213 491.213" style="enable-background:new 0 0 491.213 491.213;" xml:space="preserve">
                        <g>
                          <path d="M318.713,73.106c-32.723,0-64.571,9.208-92.103,26.628c-26.772,16.94-48.365,40.868-62.445,69.196l26.865,13.353
		c24.272-48.838,73.198-79.176,127.683-79.176c78.575,0,142.5,63.925,142.5,142.5s-63.925,142.5-142.5,142.5
		c-54.484,0-103.41-30.338-127.683-79.176l-26.865,13.353c14.08,28.328,35.673,52.256,62.445,69.196
		c27.531,17.42,59.38,26.628,92.103,26.628c95.117,0,172.5-77.383,172.5-172.5S413.83,73.106,318.713,73.106z"/>
                          <polygon points="318.713,260.606 318.713,230.606 57.426,230.606 91.819,196.213 70.606,175 0,245.607 70.606,316.213 91.819,295 
		57.425,260.606 	"/>
                        </g>
                      </svg>
                      </i><span class="link"><?php echo getLabel('L_Logout');?> </span></a></li>
                  </ul>
                </div>
              </div>

