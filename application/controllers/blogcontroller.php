...
class BlogController extends FrontController {
	...
	function __construct($model, $controller, $action) {
		...
		Syspage::addCss(array('css/frontend/blog.css'));
		Syspage::addCss(array('css/frontend/ionicons.css'), true);
		$customer_id = Members::getLoggedUserID();
		
		if($customer_id > 0)
		{
        $member = new Members();
        $this->user_details = $member->getCustomerData($customer_id);
		}
    ...
    function category($cat_slug) {
        if (empty($cat_slug)) {
            Message::addErrorMessage(getLabel('M_INVALID_REQUEST'));
    ...
    private function getSearchForm() {
        ...
        $fld = $frm->addRequiredField('', 'search', '', 'search', 'title="'.getLabel('L_Search_Text').'" placeholder="'.getLabel('L_Search').'"');
    ...
    function contribution() {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
			...
			if( !$post = Syspage::getPostedVar()){
			   Message::addErrorMessage(getLabel('M_ERROR_INVALID_REQUEST')); 
                ...
                if (!$frm->validate($post)) {
                    ...
                    if (!empty($_FILES['contribution_file_name']['name'])) {
                        if ($_FILES['contribution_file_name']['size'] > (1024 * 1024 * CONF_CONTRIBUTION_FILE_UPLOAD_SIZE)) {
                            ...
                            $frm->fill($post);
							$labelMsg = str_replace('{VAR}', CONF_CONTRIBUTION_FILE_UPLOAD_SIZE, getLabel('M_ERROR_FILE_SIZE_SHOULD_NOT_EXCEED_{VAR}_MB'));
                    ...
                    if ($error == 0) {
                        ...
                        if ($this->Blog->addContributions($post)) {
                            ...
                            Message::addMessage(getLabel('M_SUCCESS_DETAILS_SAVED'));                             
                        } else {
                            Message::addErrorMessage(getLabel('M_ERROR_DETAILS_NOT_SAVED'));
                        }
                    }
                }
            }
    ...
    function uploadContributionFile($file_tmp_name, $filename, &$response, $pathSuffix = '') {
		...
		if (!in_array(trim($file_mime_type), $accepted_files)) {
			$response = getLabel('M_ERROR_VALID_FILES_FOR_BLOG_CONTRIBUTION');
			return false;
		}
        ...
        if (!copy($file_tmp_name, CONF_INSTALLATION_PATH . 'user-uploads/' . $pathSuffix . $fname)) {
            $response = getLabel('M_ERROR_COULD_NOT_SAVE_FILE');
            return false;
        }
        $response = $fname;
        return true;
    ...
    private function getContributeForm() {
        ...
        $frm->addRequiredField('', 'contribution_author_first_name', '', 'contribution_author_first_name', 'title="'.getLabel('L_First_Name').'" placeholder="'.getLabel('L_First_Name*').'"');
        $frm->addRequiredField('', 'contribution_author_last_name', '', 'contribution_author_last_name', 'title="'.getLabel('L_Last_Name').'" placeholder="'.getLabel('L_Last_Name*').'"');
        $frm->addEmailField('', 'contribution_author_email', '', 'contribution_author_email', 'title="'.getLabel('L_Email_Address').'" placeholder="'.getLabel('L_Email_Address*').'"');
        $fld = $frm->addTextBox('', 'contribution_author_phone', '', 'contribution_author_phone', 'title="'.getLabel('L_Phone_No').'" placeholder="'.getLabel('L_Phone_No.').'"');
        $fld->requirements()->setInt();
        $fld = $frm->addFileUpload('Upload File', 'contribution_file_name', 'contribution_file_name', 'title="'.getLabel('L_Upload_File').'"');
        $fld->requirements()->setRequired();
      
		$frm->addHtml('', 'htmlNote','<div class="g-recaptcha" data-sitekey="'.CONF_RECAPTACHA_SITEKEY.'"></div>');
 
        $frm->addHiddenField('', 'contribution_user_id', $customer_id, 'contribution_user_id');
        $frm->addSubmitButton('', 'btn_submit', getLabel('L_Submit'), 'btn_submit', 'class="button button--fill button--orange"');
        return $frm;
    }
    ...
    private function getCommentForm() {
		...
		$fld = $frm->addRequiredField('', 'comment_author_name', '', 'comment_author_name', 'title="'.getLabel('L_Name').'" placeholder="'.getLabel('L_Name*').'"');
		$fld->setRequiredStarWith('none');
		//$fld->setRequiredStarPosition('before');
       
		$fld = $frm->addEmailField('', 'comment_author_email', '', 'comment_author_email', 'title="'.getLabel('L_Email').'" placeholder="'.getLabel('L_Email*').'"');
		$fld->setRequiredStarWith('none');
        
		$frm->addHtml('', 'htmlNote','<div class="g-recaptcha" data-sitekey="'.CONF_RECAPTACHA_SITEKEY.'"></div>');
        
		$fld = $frm->addTextArea('', 'comment_content', '', 'comment_content', 'class="textarea_resize" placeholder="'.getLabel('L_Comment').'" title="'.getLabel('L_Comment*').'"');
		$fld->requirements()->setRequired(true);
		$fld->setRequiredStarWith('none'); 
		
        $frm->addHiddenField('', 'comment_post_id', '', 'comment_post_id');
        $frm->addHiddenField('', 'comment_user_id', Members::getLoggedUserID(), 'comment_user_id');
        $frm->addSubmitButton('', 'btn_submit', getLabel('L_Submit'), 'btn_submit', 'class="themeBtn"');
        return $frm;
    }
    ...
    function post($post_slug) {
            ...
            if (!verifyCaptcha()) { 
                ...
                if (!$frm->validate($post)) { 
                    ...
                    if ($this->Blog->addComment($post)) {
                        ...
                        Message::addMessage(getLabel('M_SUCCESS_COMMENT_SAVED'));
                        redirectUser(generateUrl('blog', 'post', array($post_slug)));
                    } else {
                        Message::addErrorMessage(getLabel('M_ERROR_COMMENT_NOT_SAVED'));
                    }
                }
            }
        }
		...
		if($post_data){	
			$post_description = trim(subStringByWords(strip_tags(renderHtml($post_data["post_content"],true)),500));
			$post_description .= ' - '.getLabel('L_See_more_at').": ".getCurrUrl();
			
			$postImageUrl = '';
			if (!empty($post_slider_images[0]['slide_images'])) {
				$postImageUrl = generateUrl('image', 'post', array('large', $post_slider_images[0]['slide_images']));
			}
        ...
        $this->set('loggedUserId', Members::getLoggedUserID());
